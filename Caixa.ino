...
//Caixa da Torre
//ArduinoOTA - Password = caixa
//-----------------------------------------------------------------------------------------------------------------------------------
//Habilita a funcionalidade de atualização de firmware Over-The-Air (OTA) no ESP32.
#include <ArduinoOTA.h> 
//-----------------------------------------------------------------------------------------------------------------------------------
#define BLYNK_TEMPLATE_ID "TMPL2nHshOXfv"
#define BLYNK_TEMPLATE_NAME "MONDRIAN CLASS RESIDENCE"
#define BLYNK_AUTH_TOKEN "LKou_kvxqE86I-2rFdNokjuP8bxPkzRw"
//-----------------------------------------------------------------------------------------------------------------------------------
//Credenciais para as redes Wi-Fi

const char* ssid = "AM_CaixaDAgua";
const char* pass = "mondrian409";
...
String botTokenWF = "7644612484:AAED5XNZXk_QrfNtHfR7I9Ost7m5qKatHYo";
...
float emptyTankDistance = 201;   // Distância medida pelo sensor quando o tanque está vazio (em cm).
float fullTankDistance  = 55.10;    // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
const float VOLUME_TOTAL = 36137; // Volume total do tanque em litros
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
const char* firmwareURL = "https://eduardocorrea62.github.io/MONDRIAN_CLASS_RESIDENCE/Caixa.ino.bin";
...
void setup() {
   Serial.begin(115200);
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\nConectado ao Wi-Fi");
        enviarmensagemWiFi("O ESP32 da Caixa D'Água foi reinicializado com sucesso");
    } else {
...
void volumeAtual() {
    ...
    Blynk.virtualWrite(V5, String(volumeFormatado) + "L");  // Envia para o Blynk no Virtual Pin V14
...
BLYNK_WRITE(V22) { // Substitua V1 pelo pino virtual configurado no Blynk
  ...
  if (buttonState == 1) {
    ...
    enviarmensagemWiFi("Caixa D'Água: Botão acionado no Blynk WEB. Aguarde...Atualizando firmware do ESP32");
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Caixa D'Água conectada à rede " + String(ssid));
  } else {
    Serial.println("\nFalha ao conectar ao Wi-Fi.");
  }
}
...
void verificarWiFi() {
    ...
    const unsigned long wifiCheckInterval = 20000; // Verificar Wi-Fi a cada 20 segundos
...
void arduinoOTA(){                                                               
  ...
  ArduinoOTA.setHostname("Caixa_DAgua");                               // Define o nome do dispositivo para identificação no processo OTA.
  ArduinoOTA.setPassword("caixa");                                               // Define a senha de autenticação para o processo OTA.
...
void displayData() {
    ...
    if (nivelPercentual != lastNivel || statusWiFi != lastStatusWiFi) {
        ...
        oled.setCursor(1, 19);  // Define nova posição do cursor
        oled.println("Caixa D'Agua");  // Exibe o título "CAIXA"
...
void checkWaterLevel() {
    ...
    // Nível de 75%
    if (nivelPercentual >= 75) {
        Blynk.virtualWrite(V7, "O nível está dentro da normalidade.");
        Blynk.setProperty(V7, "color", "#0000FF"); // Azul
