...
float emptyTankDistance = 110;    // Distância medida pelo sensor quando o tanque está vazio (em cm).
float fullTankDistance =   25;    // Distância medida pelo sensor quando o tanque está cheio (em cm).
...
const float VOLUME_TOTAL = 34560; // Volume total do tanque em litros
...
//--------------------------------------------------------------------------------------------------------------------------------------------------
const char* firmwareURL = "https://eduardocorrea62.github.io/MONDRIAN_CLASS_RESIDENCE/Cisterna.ino.bin";
...
void setup() {
    ...
    if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\nConectado ao Wi-Fi");
        enviarmensagemWiFi("O ESP32 da Cisterna foi reinicializado com sucesso");
    } else {
...
void volumeAtual() {
    ...
    Blynk.virtualWrite(V6, String(volumeFormatado) + "L");  // Envia para o Blynk no Virtual Pin V14
...
BLYNK_WRITE(V21) { // Substitua V1 pelo pino virtual configurado no Blynk
  ...
  if (buttonState == 1) {
    ...
    enviarmensagemWiFi("Cisterna: Botão acionado no Blynk WEB. Atualizando firmware do ESP32. Aguarde...");
...
void conectarWiFi() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.println("\nConectado ao Wi-Fi!");
    enviarmensagemWiFi("REDE Wi-Fi: Cisterna conectado à rede " + String(ssid));
  } else {
    Serial.println("\nFalha ao conectar ao Wi-Fi.");
    }
}
...
void arduinoOTA() {                                                             //ATUALIZAÇÃO DO CÓDIGO DO ESP32 VIA WI-FI ATRAVÉS DO OTA(Over-the-Air)
  ...
  ArduinoOTA.setHostname("Cisterna");                                 // Define o nome do dispositivo para identificação no processo OTA.
  ArduinoOTA.setPassword("cisterna");                                           // Define a senha de autenticação para o processo OTA.
...
void displayData() {
    ...
    if (nivelPercentual != lastNivel || statusWiFi != lastStatusWiFi) {
        ...
        oled.println("Cisterna");  // Exibe o título "CAIXA"
...
void checkWaterLevel() {
    ...
    if (nivelPercentual >= 50) {
        Blynk.virtualWrite(V8, "O nível está dentro da normalidade.");
        Blynk.setProperty(V8, "color", "#0000FF"); // Azul
        ...
        String mensagem = "Nível em " + String(nivelPercentual, 1) + "%. Possivel baixa pressão de água do SEMASA e/ou consumo alto.";
        Blynk.virtualWrite(V8, mensagem);
        Blynk.setProperty(V8, "color", "#FF0000"); // Vermelho
...
void blynkVirtualWrite(){
      // ALERTA SOBRE A BÓIA
      if (distancia <= 22.8 && boiaNormal && (millis() - lastMessageTime >= messageInterval)) {
